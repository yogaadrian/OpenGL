...
void GameModels::CreateTriangleModel(const std::string& gameModelName)
{
   ...
   unsigned int deltaColor = ((unsigned int*)&vertices[0].color - (unsigned int*)&vertices[0]) * sizeof(unsigned int);

   unsigned int deltaPos = ((unsigned int*)&vertices[0].position - (unsigned int*)&vertices[0]) * sizeof(unsigned int);
   glEnableVertexAttribArray(0);
   glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)deltaPos);
   glEnableVertexAttribArray(1);
   glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)deltaColor);
